...
namespace BaoTran.Services
{
    ...
    public class MediaFileService : IMediaFileService
    {
        ...
        {
            MediaFile mediaFile = await repository.MediaFiles.GetSingleAsync(x => x.IdMediaFile == idMediaFile);
            if (mediaFile == null)
            {
                return null;
            }
        ...
        {
            MediaFile existingMediaFile = await repository.MediaFiles.GetSingleAsync(g => g.IdMediaFile == id);
        ...
        {
            MediaFile? mediaFile = await repository.MediaFiles.FirstOrDefaultAsync(u => u.IdMediaFile == id);
            if (mediaFile == null)
                return (false, "Media File not found.");
